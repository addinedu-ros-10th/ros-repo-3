...
# --- FrameGrabber: 최신 프레임만 유지하는 스레드 캡처 ---
class FrameGrabber:
...
class HSVAnalyzer:
                ...
                for detection in current_detections:
                    ...
                    if do_gesture_now:
                        ...
                        if label:
                            ...
                            if not self.call_state and label == "FARTHEST":
                                self.action = "Call"
                            ...
                            if self.call_state:
                                if label == "OK_SIGN":
                                    ...
                                    send = 3;  self.send_detection_result(2, 3, send, 0, 0)
                                elif label == "THREE":
                                    self.action = "GUIDING";     self.call_state = False
                                    send = 4;  self.send_detection_result(2, 3, send, 0, 0)
                                elif label == "OPEN":
                                    self.action = "QUIT";      self.call_state = False
                                    send = 5;  self.send_detection_result(2, 3, send, 0, 0)
                                elif label == "FIST":
                                    self.action = "STOP";      self.call_state = False
                                    send = 2;  self.send_detection_result(2, 3, send, 0, 0)
    ...
    def make_update_packet(self, transaction_id: int, function_id: int, payload: dict):
        """
    ...
    def user_result(self):
        ...
        self.make_update_packet(transaction_id, function_id, payload)


    def send_detection_result(self, cam_id, obj_type, obj_data, posx, posy):
        function_id = 2
        transaction_id = 33 
