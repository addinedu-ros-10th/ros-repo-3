...
meat_ckpt = "main/freshness/saved_models/meat_checkpoint.pt"
fruit_ckpt = "main/freshness/saved_models/fruit_checkpoint.pt"
meat_fresh_idxes = "0,1"; meat_spoil_idxes = "2"
fruit_fresh_idxes = None; fruit_spoil_idxes = None
imgsz = "640"
conf = 0.25
ema_value = 0.85
...
def main():
    ...
    detector = DualDetector(
        meat_w="best_meat.pt",  # 고기 커스텀 가중치
        fruit_w="yolov8s.pt",                        # 과일 COCO 가중치
        device=device, conf=conf, imgsz=imgsz
    )

    # 2) 분류기
    meat_cls  = FreshnessClassifier(meat_ckpt,  device)
    fruit_cls = FreshnessClassifier(fruit_ckpt, device) if fruit_ckpt else None

    meat_fresh_set  = parse_idxes(meat_fresh_idxes,  len(meat_cls.classes))
    meat_spoil_set  = parse_idxes(meat_spoil_idxes,  len(meat_cls.classes))
    fruit_fresh_set = parse_idxes(fruit_fresh_idxes, len(fruit_cls.classes)) if fruit_cls else None
    fruit_spoil_set = parse_idxes(fruit_spoil_idxes, len(fruit_cls.classes)) if fruit_cls else None
    
    # 3) 상태 (EMA / ID)
    ema = EmaBank(alpha=ema_value)
    prev_boxes = {}  # id -> xyxy
            ...
            for i, d in enumerate(dets):
                ...
                if kind == "meat":
                    ...
                    fresh_c, spoiled_c, pred_label = extract_fresh_spoiled(
                        ...
                        force_fresh_idx=meat_fresh_idxes, force_spoil_idx=meat_spoil_idxes
                    )
                    ...
                    fresh_c, spoiled_c, pred_label = extract_fresh_spoiled(
                        ...
                        force_fresh_idx=fruit_fresh_idxes, force_spoil_idx=fruit_spoil_idxes
                    )
