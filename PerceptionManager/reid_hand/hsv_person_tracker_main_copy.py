...
class HSVAnalyzer:
    ...
    def run_analysis(self):
        ...
        # ===== 성능 측정용 변수 추가 =====
        t0 = time.time()        # 전체 루프 시작 시간
        loop_count = 0          # while 루프 전체 반복 횟수
        disp_count = 0          # 실제 imshow 호출 횟수 (실제 화면 갱신 FPS)
        proc_times = []         # YOLO+후처리 한 번 돌 때 걸린 시간(ms 단위 계산용)
        # ================================

        try:
            while True:
                loop_count += 1  # 루프 한 바퀴 돌 때마다 증가
                ...
                # ✅ 처리 한 사이클 시간 기록
                proc_times.append(time.time() - proc_start)

                # 표시(부하 줄이려면 resize 해서 보여줘도 됨)
                # if frame_count % 5 == 0:
                disp_count += 1   # 실제로 화면에 그린 횟수
            ...
            overall_fps = loop_count / total_time if total_time > 0 else 0.0
            display_fps = disp_count / total_time if total_time > 0 else 0.0
            avg_proc = np.mean(proc_times) if proc_times else 0.0
            p95_proc = np.percentile(proc_times, 95) if proc_times else 0.0

            print("\n===== Performance Report (THIS CONFIG) =====")
            print(f"Total time       : {total_time:.2f} s")
            print(f"Loop iterations  : {loop_count}")
            print(f"Frames processed : {len(proc_times)}")
            print(f"Loop FPS         : {overall_fps:.2f} fps")
            print(f"Display FPS      : {display_fps:.2f} fps")
            print(f"Avg proc/frame   : {avg_proc*1000:.1f} ms")
            print(f"p95 proc/frame   : {p95_proc*1000:.1f} ms")
            print("============================================")
